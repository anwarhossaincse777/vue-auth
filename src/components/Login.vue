...
<template>
    <p class="mt-10">
        <!-- <button v-if="!auth.isAuthenticated" @click="auth.authenticate()" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
            <span v-if="auth.isAuthenticated">You're already logged in</span>
            <span v-else>Click to Login</span>
        </button>

        <button @click="auth.logout()" class="ml-5 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
            Log out
        </button> -->
    <section class="">
            ...
            <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 ">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <div class="space-y-4 md:space-y-6" action="#">
                        ...
                        <p class="text-right">
                            <button v-if="!auth.isAuthenticated" @click="auth.authenticate(username, password)" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                                <span v-if="auth.isAuthenticated">You're already logged in</span>
                                <span v-else>Login</span>
                            </button>
                            <button @click="redirect()" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                                <span>Push</span>
                            </button>
                        </p>
                    </div>
